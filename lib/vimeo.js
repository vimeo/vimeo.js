 ...
 */
Vimeo.prototype.upload = function (path, params, completeCallback, progressCallback, errorCallback) {
  var _self = this
...
Vimeo.prototype.replace = function (
  ...
  var options = {
    method: 'PUT',
    path: videoUri + '/files?fields=upload_link,complete_uri',
    query: {
      type: 'streaming'
    }
  }
  ...
  this.request(options, function (err, ticket, status, headers) {
    ...
    var file = new FileStreamer(path, ticket.upload_link, progressCallback)
    file.ready(function () {
      _self.request({
        method: 'DELETE',
