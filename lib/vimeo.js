...
Vimeo.prototype.accessToken = null
 ...
 *
 * 2. Options + callback
 *     If an object is provided, it should match the response of urlModule.parse. Path is the only required parameter.
 *
 *     hostname
 *     port
 ...
 *
 * @param {string|Object} options   String path (default GET), or object with `method`, path`, `host`, `port`, `query`
 *                                  or `headers`.
...
Vimeo.prototype.request = function (options, callback) {
  ...
  // If we don't have a path at this point, error. a path is the only required field. We have defaults for everything
  // else important.
...
Vimeo.prototype._buildRequestOptions = function (options) {
  // Set up the request object. we always use the options paramter first, and if no value is provided we fall back to
  // request defaults.
  ...
  if (this.accessToken) {
    ...
    requestOptions.headers.Authorization = 'Basic ' + new Buffer(this._clientId + ':' + this._clientSecret).toString('base64')
  }
 ...
 * @param {string}   redirectUri  The exact `redirectUri` provided to `buildAuthorizationEndpoint` and configured in your
 *                                API app settings.
 ...
 *
 * The destination allows the user to accept or deny connecting with vimeo, and accept or deny each of the scopes you
 * requested. Scopes are passed through the second parameter as an array of strings, or a space delimited list.
 *
 * Once accepted or denied, the user is redirected back to the `redirectUri`.
 *
 * @param  {string} redirectUri  The uri that will exchange a code for an access token. Must match the uri in your API
 *                               app settings.
 ...
 *
 * @param  {string}   scope An array of scopes. See https://developer.vimeo.com/api/authentication#scopes for more.
 * @param  {Function} fn    A function that is called when the request is complete. If an error occured the first
 *                          parameter will be that error, otherwise the first parameter will be null.
 */
 ...
 */
Vimeo.prototype.streamingUpload = function (path, videoUri, callback, progressCallback) {
  var _self = this

  if (typeof videoUri === 'function') {
  ...
  var options = {
    method: videoUri ? 'PUT' : 'POST',
    path: videoUri ? videoUri + '/files' : '/me/videos',
    query: {
      type: 'streaming'
    }
  }
  ...
  this.request(options, function (err, ticket, status, headers) {
    ...
    var file = new FileStreamer(path, ticket.upload_link_secure, progressCallback)
 ...
 *
 * @param {string}    path              Path to the file you wish to upload.
 ...
 */
Vimeo.prototype.tusUpload = function (path, filename, completeCallback, progressCallback, errorCallback) {
  ...
  var options = {
    ...
    query: {
      type: 'tus',
      filename: (typeof filename !== 'undefined') ? filename : path,
