 ...
 * 1. Url + Callback
 *     If a url is provided, we fill in the rest of the request options with 
 *     defaults (GET http://api.vimeo.com/{url}).
 *
 * 2. Options + callback
 *     If an object is provided, it should match the response of url_module.parse. 
 ...
 *
 * @param {String|Object} options string path (default GET), or object with path, 
 * host, port, query, headers
...
Vimeo.prototype.request = function vimeo_request(options, callback) {
    ...
    var request_options, client, req;
	...
	// if we don't have a path at this point, error. a path is the only required field. 
	...
	request_options = this._buildRequestOptions(options);
	...
	req = client.request(request_options, this._handleRequest(callback));
...
Vimeo.prototype._handleRequest = function (callback) {
	return function (res) {
		...
		if (res.statusCode >= 400) {
			// failed api calls should wait for the response to end and 
			...
			// successful api calls should wait for the response to end and then 
            // call the callback with the response body
			res.on('end', function () {
                var body = null;
				try {
					console.log('buffer');
...
Vimeo.prototype._buildRequestOptions = function (options) {
	// set up the request object. we always use the options paramter first, 
    // and if no value is provided we fall back to request defaults
	...
	if (this.access_token) {
		...
		request_options.headers.Authorization = 'Basic ' +
            new Buffer(this._client_id + ':' + this._client_secret).toString('base64');
	}
...
/**
 * Create an object of request options based on the provided list of options, 
 * and the request defaults.
 * 
 * @param  {Object} options
 * @return {Object}
 */
Vimeo.prototype._applyDefaultRequestOptions = function (options) {
	...
	if (module.exports.request_defaults.headers) {
		for (key in module.exports.request_defaults.headers) {
            if (module.exports.request_defaults.headers.hasOwnProperty(key)) {
...
Vimeo.prototype._applyQuerystringParams = function (request_options, options) {
	...
	if (Object.keys(options.query).length) {
		if (request_options.path.indexOf('?') < 0) {
			...
			// If the user already added parameters to the url, we want to add them as 
 ...
 * @param  {String} redirect_uri the exact redirect_uri provided to buildAuthorizationEndpoint and 
 *                  configured in your api app settings
 * @return {null}
 */
 ...
 *
 * @param  {String} redirect_uri The uri that will exchange a code for an access token. Must match 
 *                               the uri in your app settings.
 * @param  {String} scope        An array of scopes. see 
 *                               https://developer.vimeo.com/api/authentication#scopes for more
...
Vimeo.prototype.buildAuthorizationEndpoint = function (redirect_uri, scope, state) {
	...
	return module.exports.request_defaults.protocol + '//' +
        ...
        auth_endpoints.authorization + '?' + qs_module.stringify(query);
};
