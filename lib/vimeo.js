 ...
 * @param {Function|null} callback  Called when complete, `function (err, json)`.
 */
...
Vimeo.prototype._handleRequest = function (callback, reject) {
  return function (res) {
    ...
    console.log('_handleRequest 1')
    if (res.statusCode >= 400) {
      // Failed api calls should wait for the response to end and then call the callback with an
      // error.
      console.log('_handleRequest 2')
      res.on('end', function () {
        const err = new Error(buffer)
        reject ? reject(err) : callback(err, buffer, res.statusCode, res.headers)
      })
    } else {
      ...
      res.on('end', function () {
        try {
          ...
          return reject ? reject(e) : callback(e, buffer, res.statusCode, res.headers)
        }

        console.log('_handleRequest 3')
