 ...
 * If the upload is complete it will call _user_ready, if not it will attempt to 
...
FileStreamer.prototype._ready = function () {
    ...
    // If we think we are ready to complete, check with the server and see if 
    // they have the whole file
...
FileStreamer.prototype._putFile = function (start, callback) {
    var _self, file, headers, req;

    _self = this;

    file = fs_module.createReadStream(_self._path, {
        start : start
    });
    ...
    req = _self._upload_endpoint_request({
        method : 'PUT',
        headers : headers
    }, callback);
...
FileStreamer.prototype._getNewStart = function (next) {
    ...
    this._upload_endpoint_request({
        ...
        if (status === 308) {
            return next(null, parseInt(headers.range.split('-')[1], 10));
        }
...
FileStreamer.prototype._upload_endpoint_request = function (options, callback) {
    ...
    req = client.request(request_options);
    ...
    req.on('response', function (res) {
        ...
        if (res.statusCode > 399) {
            // failed api calls should wait for the response to end and then call 
