...
var server = httpModule.createServer(function (request, response) {
  ...
  if (url.pathname === '/oauth_callback') {
    ...
    if (!url.query.error) {
      ...
      response.end('<a href="' + lib.buildAuthorizationEndpoint(callbackUrl, scopes, 'abcdefg') + '">Link with Vimeo</a><br />' + JSON.stringify(url.query))
    }
  } else {
    if (stateData.state !== 'authorized') {
      // At this state (any request where stateData.state has not been set to "authorized")
      ...
      response.setHeader('Content-Type', 'text/html')
      response.write('Your command line is currently unauthenticated. Please ')
      response.end('<a href="' + lib.buildAuthorizationEndpoint(callbackUrl, scopes, 'abcdefg') + '">Link with Vimeo</a>')
    } else {
      // At this state (stateData.state has been set to "authorized" when we retrieved the access token)
      // we can make authenticated api requests
      console.info('http request with access token')
      response.setHeader('Content-Type', 'text/html')
      response.end('Your command line is currently authorized with the user : <a href="' + stateData.user.link + '">' + stateData.user.name + '</a>. You can make api requests via the command line using the "request" function, or upload files using the "upload" function.<br /> Try "request(\'/me\');"')
    }
  }
})
 ...
 * @param  {string} videoUri Optional. If provided, this upload will replace the source file of the video uri provided
 */
 ...
 *  - options.query is an object containing all of your request parameters. If GET they will be appended to the url, if
 *    POST it will be part of your request body
 *  - options.headers is an object containing key value pairings of all of the HTTP request headers
 *
 * @param {Object|String} options If string, it will make a GET request to that url. If an object, you can provide many
 *                                parameters. See the function description for more.
 */
